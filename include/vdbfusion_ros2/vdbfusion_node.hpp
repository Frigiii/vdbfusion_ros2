...
class vdbfusion_node : public rclcpp::Node {
 ...
 private:
  ...
  void publishTSDF(std::shared_ptr<VDBVolumeType> vdb_volume,
                   const std::string& ns = "vdbfusion");
  void publishMesh(const std::string& ns = "vdbfusion") {
    publishMesh(vdb_volume_, ns);
  }
  void publishMesh(std::shared_ptr<VDBVolumeType> vdb_volume,
                   const std::string& ns = "vdbfusion");
